...
static DLinkNode *dlink4_head = NULL;
...
void  check(DLinkNode *dp){

    dbg_print("+++++++++++++++++");
    DLinkNode *dnp = dlink4_head;
...
void dlink_delete(DLinkNode *dnp){
    ...
    if(dnp->prev == NULL){
        switch(GET_BSIZE(dnp)/ALIGNMENT){
            ...
            default:
                if(dnp == dlink4_head)
                    dlink4_head = dnp->next;
                break;
        }
       
    }
    else{
        dbg_print("dnp->prev != NULL");
    ...
    switch(GET_BSIZE(dnp)/ALIGNMENT){
        ...
        default:
            if(dlink4_head== NULL){
                ...
                dnp->next = dlink4_head;
                dlink4_head->prev = dnp;
                dnp->prev = NULL;
            }
            dlink4_head = dnp;
            break;
...
void *find_fit_block(size_t bsize){
    ...
    switch(bsize/ ALIGNMENT){
        ...
        default:
            np = dlink4_head;
            while(np!=NULL){
                dbg_print("loop %d %d", GET_BSIZE(np), bsize);
                if(GET_BSIZE(np)>=bsize)
                    break;
                else
                    np = ((DLinkNode *)np)->next;
            }
    }
    if(np!=NULL){
        bp = (char *)np + WSIZE;
...
{ 
    dbg_begin();
    heap_start = NULL;
    dlink4_head = dlink3_head = dlink2_head = slink1_head  = NULL;
